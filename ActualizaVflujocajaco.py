...
table_id = "liquid-optics-447914-a9.ENCAJESTEST.VFLUJOCAJACO"

# Tabla para los datos de Oracle (permanente)
oracle_table_id = "liquid-optics-447914-a9.ENCAJESTEST.VFLUJOCAJACO_ORACLE"  # Nombre para la tabla permanente

# Vista materializada
materialized_view_id = "liquid-optics-447914-a9.ENCAJESTEST.VFLUJOCAJACO"
...
try:
    ...
    DOCO,
    TIPODOC,
    TO_CHAR(FECHAORDEN, 'YYYY-MM-DD') AS FECHAORDEN,  -- Conversión a DATE en Oracle
    TO_CHAR(FECHARECEP, 'YYYY-MM-DD') AS FECHARECEP,  -- Conversión a DATE en Oracle
    NOMPROV,
    CANT,
    VALCOCOP,
    VALCOUSD,
...
FROM analitica.vflujocajaco
    ...
    if not df.empty:
        ...
        df['FECHAORDEN'] = pd.to_datetime(df['FECHAORDEN'], errors='coerce').dt.date
        df['FECHARECEP'] = pd.to_datetime(df['FECHARECEP'], errors='coerce').dt.date
